...
<span class="MenuboxS">
 <ul>
 <li <?php echo $timetype=='' ? 'class="hover"' : ''?> id="CheckB"><span><a href="?otype=Main">订单列表</a></span></li>
...
<script>function InfoAdd_Chose() {$.dialog.open('<?php echo site_url('common/choose_customer')?>', {title: '新增订单【第一步：选择订单任务】', width: 1080, height: 550,fixed: true}); };</script>
<script>function ChoseOK(i){{$.dialog.open('<?php echo site_url('order/add')?>?id='+i, {title: '新增订单【第二步：录入订单信息】', width: 600, height: 400,fixed: true}); };}</script>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
     ...
     <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_1">
       ...
       <tr>
         ...
         <td class="td_r_l">
			 ...
			 <option value="1" <?php echo $state==1 ? 'selected' : ''?>>未处理</option>
			 <option value="2" <?php echo $state==2 ? 'selected' : ''?>>处理中</option>
			 <option value="3" <?php echo $state==3 ? 'selected' : ''?>>已完成</option>
			 </select>
			...
			<select name='adduser' class='int'>
			 ...
			 <input type="button" class="btn1 btnxiug" value="导出" onClick=window.location.href="<?php echo site_url('order/excel')?>?<?php echo $parameter?>" /> 
			</td>
 ...
 <input type="button" class="button_info_add" value=" " title="快速添加产品"  onclick='prod_add<?php echo $row['id']?>()' />
 ...
 <script>function prod_add<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('order/product_add')?>?orderid=<?php echo $row['id']?>', {title: '添加订单', width: 600,height: 400, fixed: true}); };</script>
 <script>function View<?php echo $row['cid']?>() {$.dialog.open('<?php echo site_url('customer/order')?>?id=<?php echo $row['cid']?>', {title: '查看', width: 1220,height: 520, fixed: true}); };</script>
 ...
 <script>function Edit<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('order/edit')?>?id=<?php echo $row['id']?>', {title: '编辑', width: 600,height: 400, fixed: true}); };</script>
 <script>function Del<?php echo $row['id']?>(){art.dialog({content: '是否确定删除？',icon: 'error',ok: function () {
 $.dialog.open('<?php echo site_url('order/del')?>?id=<?php echo $row['id']?>',{title: '删除原因', width: 400,height: 150, fixed: true}); 
