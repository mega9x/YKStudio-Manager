            ...
            ="0" CLASS="table_1">
                ...
                <?php foreach ($list as $arr => $row) { ?>
                    ...
                    <script>function Edit<?php echo $row['id']?>() {
                            $.dialog.open('<?php echo site_url('order/product_edit')?>?id=<?php echo $row['id']?>', {
                                title: '编辑',
                                width: 600,
                                height: 400,
                                fixed: true
                    ...
                    <script>function Del<?php echo $row['id']?>() {
                            $.dialog({
                                content: '是否确定删除？', icon: 'error', ok: function () {
                                    art.dialog.open('<?php echo site_url('order/product_del')?>?id=<?php echo $row['id']?>');
                                    return false;
                                }, cancel: true
                            });
                        };</script>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
            <td valign="top" class="td_n Bottom_pd ">
                ...
                <input id="Back" class="btn2 btnguanb" type="button"
                       onclick="art.dialog.open.origin.location.reload();art.dialog.close();" value="关闭" name="Back">

        </tr>
    </table>
</div>
<script>function Add() {
        $.dialog.open('<?php echo site_url('order/product_add')?>?orderid=<?php echo $id?>', {
            title: '新增订单产品',
            width: 600,
            height: 400,
            fixed: true
