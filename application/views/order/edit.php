...
<form name="Save" action="<?php echo site_url('order/edit')?>?id=<?php echo $id?>" method="post" onSubmit="return CheckInput();">
<div class="cssout">
...
<?php foreach($select_linkman as $arr=>$row) {?>
 <option value="<?php echo $row['name']?>" id="<?php echo $row['tel']?>" <?php echo $row['name']==$linkman ? 'selected' :''?>><?php echo $row['name']?></option>
 <?php }?>
</select>
 &nbsp;
 <input name="Back" type="button" id="Back" class="btn1 btnxinz" value="新增" onclick='Linkmans_InfoAdd()'>
 <script>function Linkmans_InfoAdd() {$.dialog.open('<?php echo site_url('linkman/add')?>?id=<?php echo $customerid?>', {title: '新增任务链接', width: 600, height: 500,fixed: true}); };</script> 
 <script>
/*选项获取参数*/
...
document.getElementById("linktel").innerHTML=" 电话：<b style='color:red;'>"+tel+"</b>";
 ...
 下单日期</li>
 <li class="w75">
 <input name="sdate" type="text" id="oSDate" class="int Wdate" size="15" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="<?php echo $sdate?>" />
 ...
 交单日期</li>
 <li class="w75">
 <input name="edate" type="text" id="oEDate" class="int Wdate" size="15" value="<?php echo $edate?>" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" />
 </li>
