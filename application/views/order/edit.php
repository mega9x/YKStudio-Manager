    ...
    function CheckInput() {
    ...
    )
        {
            if (document.all.sdate.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '下单日期不能为空！'});
    ...
    )
        {
            if (document.all.edate.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '交单日期不能为空！'});
...
<form name="Save" action="<?php echo site_url('order/edit') ?>?id=<?php echo $id ?>" method="post"
    ...
    <div class="cssout">
        ...
        <div class="cssin">
            <ul>
                ...
                <li class="w75">
                    <?php echo $number ?> <span class="info_help help01">&nbsp;不可修改</span></li>
                    ...
                    <select name="linkman" id="oLinkman" onChange="getselectids(this.options[this.selectedIndex].id);">
                        ...
                        <?php foreach ($select_linkman as $arr => $row) { ?>
                            <option value="<?php echo $row['name'] ?>"
                                    id="<?php echo $row['tel'] ?>" <?php echo $row['name'] == $linkman ? 'selected' : '' ?>><?php echo $row['name'] ?></option>
                        <?php } ?>
                    </select>
                    &nbsp;
                ...
                <li class="w75">
                    <input name="sdate" type="text" id="oSDate" class="int Wdate" size="15"
                           onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="<?php echo $sdate ?>"/>
                ...
                <li class="w75">
                    <input name="edate" type="text" id="oEDate" class="int Wdate" size="15" value="<?php echo $edate ?>"
                ...
                <li class="w75 duohang">
                    ...
                    <textarea name="content" rows="4" id="oContent" class="int"
                              style="height:80px;width:99%;"><?php echo $content ?></textarea>
                </li>
