...
<div id="setxhing">
    ...
    <ul class="xhing">
                                    ...
                                    <?php foreach ($list as $arr => $row) { ?>
                                        <tr class="tr">
                                            ...
                                            <td class="td_l_l"><a href='#'
                                                                  onclick='Group_InfoEdit<?php echo $row['id'] ?>()'><?php echo $row['name'] ?></a>
                                            </td>
                                            <td class="td_l_c" nowrap="nowrap"><input type="button" class="btn1 btnxiug"
                                                                                      value="修改" title="修改"
                                                                                      onclick='Group_InfoEdit<?php echo $row['id'] ?>()'/>
                                                <input type="button" class="btn1 btnshanc" value="删除" title="删除"
                                                       onclick="if(confirm(' 你真的要删除吗？')) {location.href='<?php echo site_url('group/del') ?>?id=<?php echo $row['id'] ?>';};"/>
                                        ...
                                        <script>function Group_InfoEdit<?php echo $row['id']?>() {
                                                $.dialog.open('<?php echo site_url('group/edit')?>?id=<?php echo $row['id']?>', {
                                                    title: '新窗口',
                                                    width: 400,
                                                    height: 170,
                                                    fixed: true
        ...
        <script>function Group_Add() {
                $.dialog.open('<?php echo site_url('group/add')?>', {title: '新窗口', width
            :
                400, height
            :
                170, fixed
