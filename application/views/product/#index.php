...
<span class="MenuboxS">
 ...
 <script>function cp_ClassList() {$.dialog.open('<?php echo site_url('product/classlist')?>', {title: '分类管理', width: 600,height: 450, fixed: true}); };</script> 
 <script>function Add() {$.dialog.open('<?php echo site_url('product/add')?>', {title: '添加产品', width: 600,height: 400, fixed: true}); };</script>
...
<!-- <td class="td_l_c"><span style="color:red; font-weight:bold;">库存不足</span></td>-->
 ...
 <script>function Edit<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('product/edit')?>?id=<?php echo $row['id']?>', {title: '编辑产品', width: 600,height: 400, fixed: true}); };</script> 
 <script>function Del<?php echo $row['id']?>() {$.dialog( { content: '是否确定删除？',icon: 'error',ok: function () { art.dialog.open('<?php echo site_url('product/del')?>?id=<?php echo $row['id']?>');return false;},cancel: true }); };</script>
...
<input class="pagenum" id="geturl" value="<?php echo $this->lib_page->nowPage?>"><li class="last"><a href='javascript:void(0);' onclick="getValue(geturl.value)">跳转</a></li></ul>
 ...
 <script>function Setting_cp() {$.dialog.open('save3.asp?action=Setting&sType=Products', {title: '用户设置', width: 500, height: 320,fixed: true}); };</script>
</body>
</html>
