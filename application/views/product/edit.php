...
<script language="JavaScript">
    function CheckInput() {
        if (document.getElementById('pBigClass').value == "") {
            art.dialog({
                title: 'Error', time: 1, icon: 'warning'
                , content: '商品大类 不能为空！'
            });
            document.getElementById('pBigClass').focus();
            return false;
        }
        if (document.getElementById('Strade').value == "") {
            art.dialog({
                title: 'Error', time: 1, icon: 'warning'
                , content: '商品小类 不能为空！'
            });
            document.getElementById('Strade').focus();
            return false;
        }
        ...
        if (document.getElementById('pUprice').value == "") {
            art.dialog({
                title: 'Error', time: 1, icon: 'warning'
                , content: '价格 不能为空！'
            });
            document.getElementById('pUprice').focus();
            return false;
        }
    }
</script>
...
<form name="Save" action="<?php echo site_url('product/edit') ?>?id=<?php echo $id ?>" method="post">
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
            <td valign="top" class="td_n pdl10 pdr10 pdt10">
                <script language="javascript" src="<?php echo base_url() ?>theme/default/js/Ajax.js?1"></script>
 ...
 <option value="">未选择大类</option>
  ...
  foreach ($classlist as $arr => $row) {
      if ($row['parentid'] > 0) {
          ?>
          <option value="<?php echo $row['name'] ?>"
                  id="<?php echo $row['id'] ?>" <?php echo $row['name'] == $strade ? 'selected' : '' ?>><?php echo $row['name'] ?></option>
      <?php }
  } ?>
 </select>
 </span>
                    ...
                    <tr>
                        ...
                        <td class="td_l_l"><input name="title" type="text" id="pTitle" class="int" size="40"
                                                  value="<?php echo $title ?>"/></td>
                    </tr>

                    <tr>
                        <td class="td_l_r title">规格</td>
                        <td class="td_l_l"><input name="property1" type="text" id="cp_ziduan1" class="int" size="30"
                                                  value="<?php echo $property1 ?>"/></td
                        >
                    </tr>

                    <tr>
                        <td class="td_l_r title">型号</td>
                        <td class="td_l_l"><input name="property2" type="text" id="cp_ziduan2" class="int" size="30"
                                                  value="<?php echo $property2 ?>"/></td>
                    </tr>
