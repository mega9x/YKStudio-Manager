    ...
    function CheckInput() {
    ...
    )
        {
            if (document.all.type.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '跟单类型不能为空！'});
                document.all.type.focus();
                return false;
            }
        }
        if (<?php echo isset($config['bt_state']) ? 1 : 0?>=="1"
    )
        {
            if (document.all.state.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '跟单进度不能为空！'});
    ...
    )
        {
            if (document.all.linkman.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '跟单对象不能为空！'});
                document.all.linkman.focus();
                return false;
            }
        }
        if (<?php echo isset($config['bt_nexttime']) ? 1 : 0?>=="1"
    )
        {
            if (document.all.nexttime.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '下次联系不能为空！'});
                document.all.nexttime.focus();
                return false;
            }
        }
        if (0 == "1") {
            if (document.all.remind.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '提醒时间不能为空！'});
                document.all.remind.focus();
                return false;
            }
        }
    ...
    )
        {
            if (document.all.content.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '详细内容不能为空！'});
...
<form name="Save" action="<?php echo site_url('single/add') ?>" method="post" onSubmit="return CheckInput();">
    <div class="cssout">

        <div class="cssin">
            <ul>
                <li class="w100 bg"><B>新增跟单管理</B></li>
                ...
                <li class="w75">
                    ...
                    <a class="btn1 btnxinz"
                       onclick='Select_Gendan("<?php echo site_url('select/main') ?>?type=records")'>新增</a>

                    <script>function Select_Gendan(Dourl) {
                            $.dialog.open(Dourl, {title: '新增选项值', width: 850, height: 450, fixed: true});
                        };</script>
                </li>
                <li class="w25 bg cl tr">
                ...
                <li class="w75">
                    ...
                    <a class="btn1 btnxinz"
                       onclick='Select_Type("<?php echo site_url('select/main') ?>?type=type")'>新增</a>

                    <script>function Select_Type(Dourl) {
                            $.dialog.open(Dourl, {title: '新增选项值', width: 850, height: 450, fixed: true});
                        };</script>
                </li>
                <li class="w25 bg cl tr">
                ...
                <li class="w75">
                    ...
                    <input name="Back" type="button" id="Back" class="btn1 btnxinz" value="新增"
                           onclick='Linkmans_InfoAdd("<?php echo site_url('linkman/add') ?>?id=<?php echo $id ?>")'>
                    <script>function Linkmans_InfoAdd(Dourl) {
                            $.dialog.open(Dourl, {title: '新增任务链接', width: 500, height: 500, fixed: true});
                        };</script>
                    <script>
                        /*选项获取参数*/
                ...
                <li class="w75">
                    <input name="nexttime" type="text" id="rNextTime" class="int Wdate" size="22"
                ...
                <li class="w75 duohang">
                    ...
                    <textarea name="Content" rows="4" id="rContent" class="bor textarea" style="width:100%;"></textarea>
                </li>
            </ul>
        </div>
        <div class="h50b"></div>
