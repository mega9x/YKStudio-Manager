...
="0" CLASS="table_1">
 ...
 <td class="td_l_c">省份</td>
 <td class="td_l_c">市</td>
 <td class="td_l_c">选择</td>
  
 </tr>
 <?php 
 foreach($list as $arr=>$row) {
 $dname = $this->mysql_model->get_rows('area',array('id'=>$row['parentid']),'name'); 
 ?>
 <tr class="tr" style="cursor:pointer;" >
  <td class="td_l_c"><?php echo $row['id']?></td>
  <td class="td_l_c"><?php echo $dname; ?></td>
  <td class="td_l_c"><?php echo $row['name']?></td>
  <td class="td_l_c"><input type="button" class="btn1 btnxiug" onClick=window.location.href="javascript:$.dialog.open.origin.$('#area_id1').val('<?php echo $row['parentid']?>');$.dialog.open.origin.$('#area_name1').val('<?php echo $dname?>');$.dialog.open.origin.$('#area_id2').val('<?php echo $row['id']?>');$.dialog.open.origin.$('#area_name2').val('<?php echo $row['name']?>');$.dialog.close();" value="确定" title="确定" /></td>
