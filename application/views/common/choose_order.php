...
: '起始时间不能为空！'});document.all.hSdate.focus();return false;}}
 if (0=="1"){if(document.all.hEdate.value == ""){art.dialog({title: 'Error',time: 1,icon: 'warning',content
...
: '是否含税不能为空！'});document.all.hTax.focus();return false;}}
 if (1=="1"){if(document.all.hContent.value == ""){art.dialog({title: 'Error',time: 1,icon: 'warning'
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="top_bg">
 ...
 <input name="keyword" type="text" class="int" id="keyword" size="30" value="<?php echo $keyword?>" onkeyup="searchSuggest();" />
...
="0" CLASS="table_1">
 ...
 <tr class="tr" onClick=window.location.href="javascript:$.dialog.open.origin.$('#oId').val('<?php echo $row['id']?>');$.dialog.open.origin.$('#oCode').val('<?php echo $row['number']?>');$.dialog.open.origin.$('#hMoney').val('<?php echo $row['money']?>');$.dialog.open.origin.$('#qkMoney').val('<?php echo $row['qkmoney']?>');$.dialog.open.origin.$('#hRevenue').val('<?php echo $row['deposit']?>');$.dialog.close();" style="cursor:pointer;" >
