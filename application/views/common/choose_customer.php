...
<span class="MenuboxS">
<ul>
 <li <?php echo $timetype=='' ? 'class="hover"' : ''?> id="CheckB"><span><a href="?otype=Main">任务列表</a></span></li>
 <li <?php echo $timetype=='d' ? 'class="hover"' : ''?> id="CheckF"><span><a href="?timetype=d">今日新增</a></span></li>
 <li <?php echo $timetype=='w' ? 'class="hover"' : ''?> id="CheckG"><span><a href="?timetype=w">近周新增</a></span></li>
 <li <?php echo $timetype=='m' ? 'class="hover"' : ''?> id="CheckH"><span><a href="?timetype=m">本月新增</a></span></li>
</ul>
</span> 
<script>function Setting_ListAll() {$.dialog.open('save3.asp?action=Setting&sType=ListAll', {title: '
自定义设置', width: 600, height: 330,fixed: true}); };</script> 
<script>function Kehu_InfoAdd() {$.dialog.open('save1.asp?action=Kehu&sType=Add', {title: '新增任务资料', width
: 1000, height: 520,fixed: true}); };</script>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <table width="100%" border="0" cellpadding="0" cellspacing="0">
 <tr>
 <td valign="top" class="td_n pdt10"><div class="Findtip">①单击选中任务 >> ②录入信息</div></td>
 </tr>
 </table>
 ...
 <tr class="tr_b">
 <td width="40" nowrap="nowrap" class="td_l_c"><input name="chkall" type="checkbox" id="chkall" onClick
="CheckAll(this.form);getBlock('chkall','CheckSub')" value="checkbox"></td>
 ...
 <tr class="tr" onclick=window.location.href="javascript:$.dialog.open.origin.ChoseOK(<?php echo $row['id']?>);$.dialog.close();" style="cursor:pointer;" title="点击选中任务">
 <td class="td_l_c"><input type="checkbox" name="cId" id="cId<?php echo $row['id']?>" value="<?php echo $row['id']?>" onClick="getBlock('cId<?php echo $row['id']?>'
,'CheckSub');Bianse('cId<?php echo $row['id']?>')"></td>
 <td class="td_l_c"><?php echo $row['id']?></td>
 
 <td class="td_l_l"><a class="gongsim" onclick='Kehu_InfoView<?php echo $row['id']?>()' style="cursor:pointer;"><?php echo $row['name']?></a>
 </td>
 ...
 <td class="td_l_c"><font color=red><?php echo date_maktime($row['addtime'])?> </font> </td>
...
: '发送手机短信：小米 - 袁纯军 - 13477088658', width: 600,height: 400, fixed: true}); };</script> -->
 ...
 <script>function Kehu_InfoView<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('customer/infoview')?>?id=<?php echo $row['id']?>', {title: '任务名称：小米', width: 1220,height: 520, fixed: true}); };</script> 
 <script>function Kehu_InfoEdit<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('customer/edit')?>?id=<?php echo $row['id']?>', {title: '编辑', width: 1000,height: 520, fixed: true}); };</script> 
