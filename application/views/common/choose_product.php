    ...
    <tr>
        <td class="top_left td_t_n td_r_n">商品 （单击选中）
            ...
            <form name="searchForm" action="?saction=ssdetail" method="post">
                大分类：&nbsp;&nbsp;<input name="bigclass" type="text" class="int" id="bigclass" size="20"
                                          value="<?php echo $bigclass ?>"/>
                小分类：&nbsp;&nbsp;<input name="smallclass" type="text" class="int" id="smallclass" size="20"
                                          value="<?php echo $smallclass ?>"/>
                关键词搜索：&nbsp;&nbsp;<input name="keyword" type="text" class="int" id="keyword" size="20"
                                              value="<?php echo $keyword ?>"/>

                <input type="submit" name="Submit" id="ss_button" class="btn1 btnxiug" value="搜索"/> &nbsp;&nbsp;<input
                        type="button" class="btn1 btnxiug" value="清空条件" onClick=window.location.href="?"/>
            </form>
        ...
        <td valign="top" class="td_n pdl10 pdr10 pdt10">
            ...
            ="0" CLASS="table_1">
                ...
                <?php foreach ($list as $arr => $row) { ?>
                    <tr class="tr"
                        onClick=window.location.href="javascript:$.dialog.open.origin.$('#ProId').val('<?php echo $row['id'] ?>');$.dialog.open.origin.$('#ProTitle').val('<?php echo $row['title'] ?>');$.dialog.open.origin.$('#oProItemA').val('');$.dialog.open.origin.$('#oProItemB').val('');$.dialog.open.origin.$('#oProItemC').val('<?php echo $row['minqty'] ?>');$.dialog.open.origin.$('#oProItemD').val('<?php echo $row['qtys'] ?>');$.dialog.open.origin.$('#oProItemE').val('<?php echo $row['purprice'] ?>');$.dialog.open.origin.$('#oProPrice').val('<?php echo $row['saleprice'] ?>');$.dialog.open.origin.$('#oMoney').val('<?php echo $row['saleprice'] ?>');$.dialog.close();"
            ...
            <script language='javascript'>
                function getValue(obj) {
                    if (document.getElementById("geturl").value != "") {
                        location.href = "<?php echo site_url('common/choose_product')?>?page=" + escape(document.getElementById("geturl").value) + ""
                    }
                }</script>
