...
<span class="MenuboxS">
 ...
 <li <?php echo $timetype=='d' ? 'class="hover"' : ''?> id="CheckF"><span><a href="?otype=timetype&timetype=d">今日删除</a></span></li>
 <li <?php echo $timetype=='w' ? 'class="hover"' : ''?> id="CheckG"><span><a href="?otype=timetype&timetype=w">近7天删除</a></span></li>
 <li <?php echo $timetype=='m' ? 'class="hover"' : ''?> id="CheckH"><span><a href="?otype=timetype&timetype=m">本月删除</a></span></li>
 </ul>
</span>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <td width="40" class="td_l_c">
	<input name="chkall" type="checkbox" id="chkall" onClick="CheckAll(this.form);getBlock('chkall','CheckSub')" value="checkbox">
 ...
 <tr class="tr">
 <td class="td_l_c"><input type="checkbox" name="cId[]" id="cId<?php echo $row['id']?>" value="<?php echo $row['id']?>" onClick="getBlock('cId<?php echo $row['id']?>','CheckSub')"></td>
 <td class="td_l_c"><?php echo $row['id']?></td>
 <td class="td_l_l"><a onclick='Kehu_InfoView<?php echo $row['id']?>()' style="cursor:pointer;"><?php echo $row['name']?></a></td>
 <td class="td_l_c">
 <font color=red><?php echo date_maktime($row['deltime'])?> </font>
 </td>
 <td class="td_l_c"><?php echo $row['adduser']?></td>
 <td class="td_l_c">
 <input type="button" class="button_info_restore" value=" " title="撤销删除"  onclick='Recycler_InfoReDel<?php echo $row['id']?>()' />
 <input type="button" class="button_info_delete" value=" " title="彻底删除" onclick='Recycler_InfoRealDel<?php echo $row['id']?>()' />
 </td>
 </tr>
 <script>function Kehu_InfoView6() {$.dialog.open('<?php echo site_url('customer/infoview')?>?id=<?php echo $row['id']?>', {title: '查看', width: 1220,height: 520, fixed: true}); };</script> 
...
<div class="fixed_bg"> 
 ...
 <script language='javascript'>function getValue(obj){if (document.getElementById("geturl").value!="") {location.href="<?php echo site_url('recycle')?>?page="+escape(document.getElementById("geturl").value)+""}}</script>
