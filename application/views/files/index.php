...
<script>function Add() {$.dialog.open('<?php echo site_url('files/add')?>', {title: '新增', width: 500, height: 280,fixed: true}); };</script>
     ...
     <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_1">
       ...
       <tr>
         ...
         <td class="td_r_l">
			 ...
			 <option value="2" <?php echo $isshare==2 ? 'selected' :''?>>是</option>
			 <option value="1" <?php echo $isshare==1 ? 'selected' :''?>>否</option>
			 </select>
			 
            <select name="class" id="Select_SoftClass"><option value="">文件类型</option>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
 ...
 <tr class="tr_b">
 <td class="td_l_c"><input name="chkall" type="checkbox" id="chkall" onClick="CheckAll(this.form);getBlock('chkall','CheckSub')" value="checkbox"></td>
 ...
 <tr class="tr">
 <td class="td_l_c"><input type="checkbox" name="sId" id="sId<?php echo $row['id']?>" value="<?php echo $row['id']?>" onClick="getBlock('sId<?php echo $row['id']?>','CheckSub'
)"></td>
 <td class="td_l_c"><?php echo $row['id']?></td>
 <td class="td_l_c"><?php echo $row['class']?></td>
 <td class="td_l_l"><?php echo $row['title']?></td>
 
 <td class="td_l_c"><a href="<?php echo site_url('files/down')?>?id=<?php echo $row['id']?>">下载</a></td>
 
 <td class="td_l_c"><a href="<?php echo site_url('files/share')?>?id=<?php echo $row['id']?>&isshare=<?php echo $row['isshare']==1 ? 2 :1?>&PN=0" style="color:#f00;"><img src="<?php echo base_url()?>theme/default/images/ico/<?php echo $row['isshare']==1 ? 'yes' :'no'?>.gif" border=0></a></td>
 <td class="td_l_c"><?php echo $row['adduser']?></td>
 <td class="td_l_c"><?php echo $row['addtime']?></td>
  
 <td class="td_l_c"><input type="button" class="btn1 btnxiug" value="修改" title="修改" onclick='Edit<?php echo $row['id']?>()' />
 <input type="button" class="btn1 btnshanc" value="删除" title="删除" onclick='Del<?php echo $row['id']?>()' /></td>

 </tr>
 <script>function Edit<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('files/edit')?>?id=<?php echo $row['id']?>', {title: '查看', width: 500,height: 280, fixed: true}); };</script> 
