...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
 <tr>
 <td colspan=2 class="ss_All td_n"><form name="searchForm" method="post" action="?Action=ssdetail">
 ...
 <input type="button" name="button" class="btn1 btnqingk2" value=" 清空条件 " onClick=window.location.href="?action=killSession" />
 </form></td>
 </tr>
 <tr>
 <td valign="top" class="td_n pdl10 pdr10 "><table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
 <tr class="tr_t">
 ...
 <td class="td_l_l"><a href='javascript:void(0)' onclick='Kehu_InfoView<?php echo $row['customerid']?>()'><?php echo $row['customername']?></a></td>
 <td class="td_l_c"><?php echo $row['action']?></td>
 <td class="td_l_c"><?php echo $row['module']?></td>
 <td class="td_l_c"><input class="btn1 btnxinz" type="button" onclick="View<?php echo $row['id']?>()" value="查看"></td>
 <td class="td_l_c"><?php echo $row['adduser']?></td>
 <td class="td_l_c"><input type="button" class="btn1 btnshanc" value="删除" title="删除" onclick="art.dialog({content: '是否确定删除？',icon: 'error',ok: function () {art.dialog.open('<?php echo site_url('userlog/del')?>?id=<?php echo $row['id']?>');art.dialog.close();},cancelVal: '关闭',cancel: true })"  /></td>
 </tr>
 <script>function Kehu_InfoView<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('customer/infoview')?>?id=<?php echo $row['cid']?>', {title: '查看', width: 1000,height: 520, fixed: true}); };</script> 
...
<div class="fixed_bg"> <span class="r">
 <input name="Back" type="button" id="Back" class="btn1 btnpilsc" value="清空记录" onClick=window.location.href="<?php echo site_url('userlog/clear')?>">
 </span> 
 ...
 <script language='javascript'>function getValue(obj){if (document.getElementById("geturl").value!="") {location.href="<?php echo site_url('userlog')?>?page="+escape(document.getElementById("geturl").value)+""}}</script>
