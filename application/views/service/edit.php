...
<script language="JavaScript">
    ...
    function CheckInput() {
        if (<?php echo isset($value['bt_sh_proid']) ? 1 : 0?>=="1"
    )
        {
            if (document.all.ProId.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '相关产品不能为空！'});
                document.all.ProId.focus();
                return false;
            }
        }
        if (<?php echo isset($value['bt_title']) ? 1 : 0?>=="1"
    )
        {
            if (document.all.title.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '反馈主题不能为空！'});
    ...
    )
        {
            if (document.all.type.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '反馈分类不能为空！'});
                document.all.type.focus();
                return false;
            }
        }
        if (<?php echo isset($value['bt_linkman']) ? 1 : 0?>=="1"
    )
        {
    ...
    )
        {
            if (document.all.sdate.value == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '反馈日期不能为空！'});
...
<form name="Save" action="<?php echo site_url('service/edit') ?>?id=<?php echo $id ?>" method="post"
            ...
            <td valign="top" class="td_n pdl10 pdr10 pdt10">
                ...
                ="0" CLASS="table_1">
                    ...
                    <tr>
                        ...
                        <td class="td_r_l"><input name="title" type="text" id="sTitle" class="int" size="30"
                    ...
                    <tr>
                        ...
                        <td class="td_r_l"><select name="type" id="Select_Shouhou">
                            ...
                            <a class="btn1 btnxinz"
                               onclick='Select_Shouhou("<?php echo site_url('select/main') ?>?type=service")'>新增</a>
                            <script>function Select_Shouhou(Dourl) {
                                    $.dialog.open(Dourl, {title: '新增选项值', width: 850, height: 400, fixed: true});
                                };</script>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="td_r_l">
                            <select name="linkman" id="sLinkman"
                                    onChange="getselectids(this.options[this.selectedIndex].id);">
                                <option value="">请选择</option>
                                <?php foreach ($select_linkman as $arr => $row) {
                                    ...
                                    ?>
                                    <option value="<?php echo $row['name'] ?>" <?php echo $row['name'] == $linkman ? 'selected' : '' ?>
                                            id="<?php echo $row['tel'] ?>"><?php echo $row['name'] ?></option>
                                <?php } ?>
                            </select>
                            &nbsp;
                            <input name="Back" type="button" id="Back" class="btn1 btnxinz" value="新增"
                                   onclick='Linkmans_InfoAdd()'>
                            <script>function Linkmans_InfoAdd() {
                                    $.dialog.open('<?php echo site_url('linkman/add')?>?id=<?php echo $customerid?>', {
                                        title: '新窗口',
                                        width: 800,
                                        height: 340,
                                        fixed: true
 ...
 电话：<b style='color:red;'><?php echo isset($tel) ? $tel : '' ?></b>
                    ...
                    <tr>
                        ...
                        <td class="td_r_l"><input name="sdate" type="text" id="sSDate" class="Wdate int" size="15"
                    ...
                    <tr>
                        ...
                        <td class="td_r_l" style="padding:5px 10px;"><textarea name="content" rows="4" id="sContent"
                                                                               class="int"
                    ...
                    <tr>
                        ...
                        <td class="td_r_l"><input name="edate" type="text" class="int" id="sEDate"
                                                  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"
                                                  value="<?php echo $edate ?>" size="15" readonly="readonly"/></td>
                    </tr>
                    <tr>
                        <td class="td_l_r title">处理结果</td>
                        <td class="td_r_l" style="padding:5px 10px;"><textarea name="info" rows="4" id="sInfo"
                                                                               class="int"
                                                                               style="height:50px;width:98%;"><?php echo $info ?></textarea>
                        </td>
                    </tr>
                </table>
            </td>
