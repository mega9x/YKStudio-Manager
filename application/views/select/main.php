                        ...
                        <div class="MenuboxL">
                            <ul>
                                <?php foreach ($selecttype as $arr => $row) { ?>
                                    <li <?php echo $type == $arr ? 'class="hover"' : '' ?>><span><a
                                                    href="<?php echo site_url('select/main') ?>?type=<?php echo $arr ?>"><?php echo $row ?></a></span>
                                    </li>
                                <?php } ?>
                                    ...
                                    <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                        <tr>
                                            <td valign="top" class="td_n pdr10 pdt10">
                                                <form name="menuForm"
                                                      action="<?php echo site_url('select/main') ?>?action=save&type=<?php echo $type ?>&id=<?php echo $id ?>"
                                                      method="post" onSubmit="return checkInput('menuForm');">

                                                    <table width="100%" border="0" cellspacing="0" cellpadding="0"
                                                           CLASS="table_1">
                                                        <col width="100"/>
                                                        <tr class="tr_t">
                                                            <td class="td_l_l" COLSPAN="4">
                                                                <B><?php echo $action == 'edit' ? '修改' : '添加' ?></B>
                                                            </td>
                                                        </tr>
                                                        <tr class="tr">
                                                            <td class="td_l_c title">参　数</td>
                                                            <td class="td_r_l"><input name="name" type="text"
                                                                                      class="int" id="otypedata"
                                                                                      size="30" maxlength="16"
                                                                                      value="<?php echo $name ?>"></td>
                                                        </tr>
                                                        <tr>
                                                            <td colspan="4" align="right" class="td_r_l"><input
                                                                        type="submit" id="submit" class="btn2 btnbaoc"
                                                                        value="保存"></td>
                                                        </tr>
                                                    </table>
                                                </form>
                            ...
                            <tr>
                                <td valign="top" style="padding:10px;" class="td_n">
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1"
                                        ...
                                        <tr class="tr">
                                            <td class="td_l_l"><a
                                                        href="<?php echo site_url('select/main') ?>?action=edit&type=<?php echo $row['type'] ?>&name=<?php echo $row['name'] ?>"><?php echo $row['name'] ?></a>
                                            </td>
                                            <td class="td_l_c" nowrap="nowrap">
                                                <input type="button" class="btn1 btnxiug" value="修改" title="修改"
                                                       onClick="window.location.href='<?php echo site_url('select/main') ?>?action=edit&id=<?php echo $row['id'] ?>&type=<?php echo $row['type'] ?>&name=<?php echo $row['name'] ?>'"/>
                                                <input type="button" class="btn1 btnshanc" value="删除" title="删除"
                                                       onClick="location.href='<?php echo site_url('select/main') ?>?action=delete&type=<?php echo $row['type'] ?>&id=<?php echo $row['id'] ?>'"/>
                                            </td>
                                            <?php } ?>
