...
<ul>
 ...
 <li <?php echo $timetype=='d' ? 'class="hover"' : ''?> id="CheckF"><span><a href="?timetype=d">今日收支</a></span></li>
 <li <?php echo $timetype=='w' ? 'class="hover"' : ''?> id="CheckG"><span><a href="?timetype=w">近7天收支</a></span></li>
 <li <?php echo $timetype=='m' ? 'class="hover"' : ''?> id="CheckH"><span><a href="?timetype=m">本月收支</a></span></li>
...
<script>function Setting_Expense() {$.dialog.open('<?php echo site_url('setting/config')?>?type=financier', {title: '新增财务记录', width: 600, height: 400,fixed: true}); };</script>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
     ...
     <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_1">
       ...
       <tr>
         ...
         <td class="td_r_l">
			 ...
			 <option value="1" <?php echo $outin==1 ? 'selected' :''?>>收入</option>
			 <option value="2" <?php echo $outin==2 ? 'selected' :''?>>支出</option>
			 </select>
			 ...
			 <input type="button" class="btn1 btnxiug" value="导出" onClick=window.location.href="<?php echo site_url('financier/excel')?>?<?php echo $parameter?>" /> 
			</td>
 ...
 <tr>
 <td valign="top" colspan=2 style="padding:10px 10px 0;" class="td_n"><form name="Search" action="?action=CheckSub&saction=Search&PN=1" method="post">
 ...
 <td class="td_l_l">
 <a onclick='Kehu_InfoView<?php echo $row['customerid']?>()' style="cursor:pointer;"><?php echo $row['customername'] ? $row['customername'] : '无公司资料关联'?></a>
 </td>
 ...
 <script>function Kehu_InfoView<?php echo $row['cid']?>() {$.dialog.open('<?php echo site_url('customer/expense')?>?id=<?php echo $row['cid']?>', {title: '查看', width: 1220,height: 520, fixed: true}); };</script> 
 <script>function Edit<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('financier/edit')?>?id=<?php echo $row['id']?>', {title: '编辑', width: 600,height: 400, fixed: true}); };</script> 
 <script>function Del<?php echo $row['id']?>(){art.dialog({content: '是否确定删除？',icon: 'error',ok: function () {
 art.dialog.open('<?php echo site_url('financier/del')?>?id=<?php echo $row['id']?>',{title: '删除原因', width: 400,height: 150, fixed: true});
...
<script>function Add_IN() {$.dialog.open('<?php echo site_url('financier/add')?>?outin=1', {title: '新增收入记录', width: 600, height: 400,fixed: true}); };</script> 
<script>function Add_OUT() {$.dialog.open('<?php echo site_url('financier/add')?>?outin=2', {title: '新增支出记录', width: 600, height: 400,fixed: true}); };</script>
