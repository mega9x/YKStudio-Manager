...
<script>function Setting_Gendan() {
        $.dialog.open('<?php echo site_url('setting/config')?>?type=single', {
            title: '自定义设置',
            width: 600,
            height: 290,
            fixed: true
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
                ...
                <?php foreach ($list as $arr => $row) { ?>
                    ...
                    <script>function Edit<?php echo $row['id']?>() {
                            $.dialog.open('<?php echo site_url('single/edit')?>?id=<?php echo $row['id']?>', {
                                title: '修改跟单信息',
                                width: 600,
                                height: 400,
                                fixed: true
                    ...
                    <script>function Del<?php echo $row['id']?>() {
                            $.dialog({
                                content: '是否确定删除？', icon: 'error', ok: function () {
                                    art.dialog.open('<?php echo site_url('single/del')?>?id=<?php echo $row['id']?>');
                                    return false;
                                }, cancel: true
                            });
                        };</script>
...
<script>function Add() {
        $.dialog.open('<?php echo site_url('single/add')?>?id=<?php echo $id?>', {
            title: '新窗口',
            width: 600,
            height: 400,
            fixed: true
