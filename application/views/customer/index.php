...
<ul>
 ...
 <li <?php echo $timetype=='d' ? 'class="hover"' : ''?> id="CheckF"><span><a href="?timetype=d">今日新增</a></span></li>
 <li <?php echo $timetype=='w' ? 'class="hover"' : ''?> id="CheckG"><span><a href="?timetype=w">近7天新增</a></span></li>
 <li <?php echo $timetype=='m' ? 'class="hover"' : ''?> id="CheckH"><span><a href="?timetype=m">本月新增</a></span></li>
...
<script>function setting_diy() {$.dialog.open('<?php echo site_url('setting/config')?>?type=customer', {title: '自定义设置', width: 600, height: 330,fixed: true}); };</script> 
<script>function add() {$.dialog.open('<?php echo site_url('customer/add')?>', {title: '新增任务资料', width: 1000, height: 520,fixed: true}); };</script>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
       ...
       <tr>
         ...
         <td class="td_r_l">
             ...
             <input name="enddate" type="text" maxlength="10" id="TimeEnd" class="int Wdate" size="10" onfocus="WdatePicker()" value="<?php echo $enddate?>" /> 
		      ...
		      <select name="type" id="Select_Type"><option value="">转化情况</option>
             ...
             <select name="start" id="Select_Star"><option value="">流量状况</option>
             ...
             <select name="source" id="Select_source"><option value="">所属联盟</option>
			 ...
			 <?php }?>
			  ...
			  <select name='adduser' class='int'><option value="">工号</option>
			...
			<input name="area1" type="text" class="int" id="area_name1"  value="" size="15" readonly onclick='choose_area1()'>	
			...
			<script>function choose_area2() {$.dialog.open('<?php echo site_url('common/choose_area2')?>', {title:'选择市', width: 700, height: 520,fixed: true}); };</script> 
			 ...
			 <input type="button" class="btn1 btnxiug" value="导出" onClick=window.location.href="<?php echo site_url('customer/excel')?>?<?php echo $parameter?>" /> 
			</td>
 ...
 <tr>
 <td valign="top" colspan=2 style="padding:5px 10px 0;" class="td_n"><form name="Search" action="<?php echo site_url('customer/other')?>?action=CheckSub&saction=Search&PN=1" method="post">
 <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_1">
 <tr class="tr_b">
 <td width="40" nowrap="nowrap" class="td_l_c"><input name="chkall" type="checkbox" id="chkall" onClick="CheckAll(this.form);getBlock('chkall','CheckSub')" value="checkbox"></td>
 <td width="50" nowrap="nowrap" class="td_l_c">编号</td>
  
 <td width="150" nowrap="nowrap" class="td_l_l">任务名称</td>
 ...
 <td width="120" nowrap="nowrap" class="td_l_c">任务要求</td>
 <?php }?>
 ...
 <?php }?>
     ...
     <?php if (isset($config['record']) && $this->session->userdata('roleid') != 3){?>
         <td width="180" nowrap="nowrap" class="td_l_c">任务代码</td>
     <?php }?>

 <?php if (isset($config['linkman'])){?>
 <td width="120" nowrap="nowrap" class="td_l_c">任务链接</td>
 <?php }?>
 <?php if (isset($config['mobile']) && $this->session->userdata('roleid') != 3){?>
 <td width="120" nowrap="nowrap" class="td_l_c">合作站点</td>
 <?php }?>
...
<?php }?>
 ...
 <tr class="tr">
 <td class="td_l_c"><input type="checkbox" name="cId" id="cId" value="<?php echo $row['id']?>"></td>
 <td class="td_l_c"><?php echo $row['id']?></td>
  
 <td class="td_l_l vtip" title="任务链接：<?php echo $row['linkman_num']?><br>跟单：<?php echo $row['single_num']?><br>订单：<?php echo $row['order_num']?><br>合同：<?php echo $row['contract_num']?><br>售后：<?php echo $row['service_num']?><br>附件：<?php echo $row['file_num']?><br>"><a class="gongsim" onclick='view<?php echo $row['id']?>()' style="cursor:pointer;"><?php echo $row['name']?></a></td>
  
 <?php if (isset($config['area'])){?>
 <td class="td_l_c"><?php echo $row['area1']?>&nbsp;&nbsp;<?php echo $row['area2']?></td>
 <?php }?>
 <?php if (isset($config['address']) && $this->session->userdata('roleid') != 3){?>
 <td class="td_l_c" onMouseOver="tip.start(this)" title="<?php echo $row['address']?>"><?php echo $row['address']?></td>
 <?php }?>
 <?php if (isset($config['type'])){?>
 ...
 <?php }?>
        <?php if (isset($config['record']) && $this->session->userdata('roleid') != 3){?>
		  <td class="td_l_l vtip"><?php
		  echo "<textarea readonly='readonly'>" .htmlentities($row['record']). "</textarea>"
              ?>
		  </td>
<?php }?>
<?php if (isset($config['linkman'])){?>
 ...
 <td style="display: "none";" class="td_l_c"><?php echo $row['job']?></td>
 <?php }?>
 ...
 $.dialog.open('<?php echo site_url('customer/infoview')?>?id=<?php echo $row['id']?>', {title: '任务名称：<?php echo $row['name']?>', width: 1220,height: 520, fixed: true}); };
 </script> 
 <script>function edit<?php echo $row['id']?>() {
 $.dialog.open('<?php echo site_url('customer/edit')?>?id=<?php echo $row['id']?>', {title: '编辑', width: 1000,height: 520, fixed: true}); };</script> 
 ...
 art.dialog.open('<?php echo site_url('customer/del')?>?id=<?php echo $row['id']?>',{title: '删除原因', width: 400,height: 150, fixed: true});
