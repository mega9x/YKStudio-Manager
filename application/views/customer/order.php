 ...
 <?php } ?>
     ...
     <?php if (in_array(34, $lever)) { ?>
         <li class="hover"><span><a
                         href="<?php echo site_url('customer/order') ?>?id=<?php echo $id ?>">订单记录</a></span></li>
     <?php } ?>
        ...
        <td valign="top" class="td_n pdl10 pdr10 pdt10">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
                ...
                <?php foreach ($list as $arr => $row) { ?>
                    <tr class="tr">
                        <td class="td_l_c"><a title="查看订单"
                                              onclick='Edit<?php echo $row['id'] ?>()'><?php echo $row['number'] ?></a>
                        </td>
                        <?php if (isset($value['linkman'])) { ?>
                        ...
                        <?php if (isset($value['content'])) { ?>
                            <td class="td_l_c">
                                <input type="button" class="btn1 btnchak" value="查看"
                                       onclick='View<?php echo $row['id'] ?>()'/>
                            </td>
                        <?php } ?>
                        <?php if (isset($value['adduser'])) { ?>
                    ...
                    <script>function prod_add<?php echo $row['id']?>() {
                            $.dialog.open('<?php echo site_url('order/product_add')?>?oid=<?php echo $row['id']?>', {
                                title: '添加',
                                width: 600,
                                height: 500,
                                fixed: true
                    ...
                    <script>function Del<?php echo $row['id']?>() {
                            $.dialog({
                                content: '是否确定删除？', icon: 'error', ok: function () {
                                    art.dialog.open('<?php echo site_url('order/del')?>?id=<?php echo $row['id']?>');
                                    return false;
                                }, cancel: true
                            });
                        };</script>
...
<script>function Add() {
        $.dialog.open('<?php echo site_url('order/add')?>?id=<?php echo $id?>', {
            title: '新窗口',
            width: 800,
            height: 340,
            fixed: true
