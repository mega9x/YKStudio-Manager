...
<div id="setxhing">
            ...
            <tr>
                <td colspan=2 class="ss_All td_n">
                    <form name="searchForm" method="post" action="<?php echo site_url('userloginlog') ?>">
                        <input name="sttdate" type="text" id="TimeBegin" class="int Wdate"
                               value="<?php echo $sttdate ?>" style="width:100px;" onFocus="WdatePicker()"/>
                        &nbsp;~&nbsp;
                            ...
                            <script>function Del<?php echo $row['id']?>() {
                                    $.dialog({
                                        content: '是否确定删除？', icon: 'error', ok: function () {
                                            art.dialog.open('<?php echo site_url('userloginlog/del')?>?id=<?php echo $row['id']?>');
                                            art.dialog.close();
                                        }, cancel: true
                                    });
                                };</script>
 ...
 <input name="Back" type="button" id="Back" class="btn1 btnpilsc" value="清空记录"
        onClick=window.location.href="<?php echo site_url('userloginlog/clear') ?>">
 </span>
                ...
                <ul class="pagination">
                    ...
                    <script language='javascript'>function getValue(obj) {
                            if (document.getElementById("geturl").value != "") {
                                location.href = "<?php echo site_url('userloginlog')?>?page=" + escape(document.getElementById("geturl").value) + ""
                            }
                        }</script>
    ...
    <ul class="xhing">
        <iframe src="<?php echo site_url('userlog') ?>" frameborder="0" marginheight="0" marginwidth="0" frameborder="0"
                scrolling="auto" width="100%" height="550">
        </iframe>
    </ul>
</div>
