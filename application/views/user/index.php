...
<script>function User_Add() {
        $.dialog.open('<?php echo site_url('user/add')?>?action=User&sType=Add', {
            title: '新窗口',
            width: 800,
            height: 400,
            fixed: true
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
                ...
                <?php foreach ($list as $arr => $row) { ?>
                    <tr class="tr">
                        ...
                        <td class="td_l_l"><a href='#'
                                              onclick='User_InfoEdit<?php echo $row['uid'] ?>()'><?php echo $row['username'] ?></a>
                        </td>
                        ...
                        <td nowrap="nowrap" class="td_l_c">
                            ...
                            <?php if ($row['roleid'] > 0) { ?>
                                <input type="button" class="button_info_quanxian" value="权限" title="用户权限"
                                       onclick='User_Quxnain<?php echo $row['uid'] ?>()'/>
                                <input type="button" class="btn1 btnxiug" value="修改" title="修改"
                                       onclick='User_InfoEdit<?php echo $row['uid'] ?>()'/>
                                <input type="button" class="btn1 btnshanc" value="删除" title="删除"
                                       onClick="art.dialog({content: '是否确定删除？',icon: 'error',ok: function () {art.dialog.open('<?php echo site_url('user/del') ?>?id=<?php echo $row['uid'] ?>');art.dialog.close();},cancelVal: '关闭',cancel: true })"/>
                            <?php } else { ?>
                                <input class="btn1 btnshanc" type="button" title="超级管理员不可修改和删除!"
                                       value="超级管理员" style="background:#757777;">
                            <?php } ?>

                        </td>
                    </tr>
                    <script>function User_Quxnain<?php echo $row['uid']?>() {
                            $.dialog.open('<?php echo site_url('user/lever')?>?id=<?php echo $row['uid']?>', {
                                title: '新窗口',
                                width: 800,
                                height: 550,
                                fixed: true
                    ...
                    <script>function User_InfoEdit<?php echo $row['uid']?>() {
                            $.dialog.open('<?php echo site_url('user/edit')?>?id=<?php echo $row['uid']?>', {
                                title: '新窗口',
                                width: 800,
                                height: 400,
                                fixed: true
