...
<span class="MenuboxS">
 ...
 <li <?php echo $timetype=='' ? 'class="hover"' : ''?> id="CheckB"><span><a href="?otype=Main">合同列表</a></span></li>
 <li <?php echo $timetype=='d' ? 'class="hover"' : ''?> id="CheckF"><span><a href="?otype=timetype&timetype=d">今日新增</a></span></li>
 <li <?php echo $timetype=='w' ? 'class="hover"' : ''?> id="CheckG"><span><a href="?otype=timetype&timetype=w">近7天新增</a></span></li>
 <li <?php echo $timetype=='m' ? 'class="hover"' : ''?> id="CheckH"><span><a href="?otype=timetype&timetype=m">本月新增</a></span></li>
 <li <?php echo $timetype=='d10' ? 'class="hover"' : ''?> id="CheckF"><span><a href="?otype=timetype&timetype=d10">10天内到期</a></span></li>
 <li <?php echo $timetype=='m1' ? 'class="hover"' : ''?> id="CheckH"><span><a href="?otype=timetype&timetype=m1">本月到期</a></span></li>
 <?php if ($this->common_model->check_lever(40)){?> 
 <li class="btn_new" id="CheckC"><span><a href="#" onclick='InfoAdd_Chose()'>新增合同</a></span></li>
 <?php }?>
 </ul>
</span>
<script>function InfoAdd_Chose() {$.dialog.open('<?php echo site_url('common/choose_customer')?>', {title: '新增合同【第一步：选择合同任务】', width: 1080, height: 550,fixed: true}); };</script>
<script>function ChoseOK(i){
{$.dialog.open('<?php echo site_url('contract/add')?>?id='+i, {title: '新增合同【第二步：录入合同信息】', width: 600, height: 480,fixed: true}); };
}
</script>


<script>function Setting_Hetong() {$.dialog.open('<?php echo site_url('setting/config')?>?type=contract', {title: '自定义设置', width: 600, height: 370,fixed: true}); };</script>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
     ...
     <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_1">
       ...
       <tr>
         ...
         <td class="td_r_l">
			 ...
			 <option value="3" <?php echo $hState=='3' ? 'selected' : ''?>>待审</option>
			 <option value="1" <?php echo $hState=='1' ? 'selected' : ''?>>合同有效</option>
			 <option value="2" <?php echo $hState=='2' ? 'selected' : ''?>>合同无效</option>
			 </select>
			 ...
			 <input type="button" class="btn1 btnxiug" value="导出" onClick=window.location.href="<?php echo site_url('contract/excel')?>?<?php echo $parameter?>" /> 
			</td>
 ...
 <td class="td_l_l"><a onclick='View<?php echo $row['customerid']?>()' style="cursor:pointer;"><?php echo $row['customername']?></a></td>
 <?php if (isset($config['number'])){?>
 <td class="td_l_c">
  
  <a  onclick='Edit<?php echo $row['id']?>()' style="color:red;cursor:pointer"><?php echo $row['number']?></a>
 ...
 <td class="td_l_c">
<a class="btn_sh<?php echo $row['state']?>" onclick='Audit<?php echo $row['id']?>()'><?php echo $audit[$row['state']]?></a>
 </td>
 ...
 <input name="Back" type="button" id="Back" class="btn1 btnxinz" value="收款" onclick='Hetong_Revenue_InfoAdd<?php echo $row['id']?>()'>
 ...
 <?php }?>
  ...
  <script>function Hetong_Revenue_InfoAdd<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('contract/addrevenue')?>?id=<?php echo$row['id']?>', {title: '新窗口', width: 400, height: 200,fixed: true}); };</script></td>
 <script>function hetong_xufei_InfoAdd<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('contract/addrenew')?>?id=<?php echo $row['id']?>', {title: '续费', width: 600,height: 560, fixed: true}); };</script>
 <script>function View<?php echo $row['cid']?>() {$.dialog.open('<?php echo site_url('customer/hetong')?>?id=<?php echo $row['cid']?>', {title: '查看', width: 1220,height: 520, fixed: true}); };</script>
 <script>function hetong_xufei_List<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('contract/renewlist')?>?id=<?php echo $row['id']?>', {title: '查看', width: 860,height: 440, fixed: true}); };</script>
 <script>function order_cp_List() {$.dialog.open('save2.asp?action=Ordercp&sType=List&Id=', {title: '查看订单产品明细', width: 860,height: 440, fixed: true}); };</script>
 <script>function Edit<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('contract/edit')?>?id=<?php echo $row['id']?>', {title: '编辑', width: 600,height: 560, fixed: true}); };</script>
 <script>function Audit<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('contract/audit')?>?id=<?php echo $row['id']?>', {title: '审核', width: 400,height: 180, fixed: true}); };</script>
 <script>function Del<?php echo $row['id']?>(){art.dialog({content: '是否确定删除？',icon: 'error',ok: function () {
 $.dialog.open('<?php echo site_url('contract/del')?>?id=<?php echo $row['id']?>',{title: '删除原因', width: 400,height: 150, fixed: true}); 
