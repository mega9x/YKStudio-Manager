...
<script>function Setting_Linkmans() {$.dialog.open('<?php echo site_url('setting/config')?>?type=linkmans', {title: '自定义设置', width: 600, height: 360,fixed: true}); };</script> 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <script>function Edit<?php echo $row['id']?>() {$.dialog.open('<?php echo site_url('linkman/edit')?>?id=<?php echo $row['id']?>', {title: '编辑', width: 800,height: 340, fixed: true}); };</script> 
 <script>function Del<?php echo $row['id']?>() {$.dialog( { content: '是否确定删除？',icon: 'error',ok: function () { art.dialog.open('<?php echo site_url('linkman/del')?>?id=<?php echo $row['id']?>');return false;},cancel: true }); };</script> 
 <script>function View<?php echo $row['id']?>() {art.dialog({ title: '详情备注',content: '<?php echo $row['content']?>',drag: false,resize: false}); };</script>
 <?php }?>
...
<script>function Add() {$.dialog.open('<?php echo site_url('linkman/add')?>?id=<?php echo $id?>', {title: '新窗口', width: 900, height: 420,fixed: true}); };</script>
