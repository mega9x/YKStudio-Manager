...
<span class="MenuboxS">
 ...
 <li class="hover"><span><a href="<?php echo site_url('customer/files')?>?id=<?php echo $id?>">附件记录</a></span></li>
 <?php }?>
 ...
 <td class="td_l_c"><input  class="btn1 btnchak" type="button" onclick=window.location.href="<?php echo site_url('customer/files_down')?>?id=<?php echo $row['id']?>" value="下载"></td>
 <td class="td_l_c"><input type="button"  class="btn1 btnchak" value="查看" onclick='File_ContentView<?php echo $row['id']?>()' /></td>
 <td class="td_l_c"><?php echo $row['adduser']?></td>
 <td class="td_l_c"><?php echo $row['adddate']?></td>
 <td class="td_l_c"><input type="button" class="button_info_del" value="删除" title="删除" onclick='Del<?php echo $row['id']?>()' /></td>
 </tr>
 <script>function Del<?php echo $row['id']?>() {$.dialog( { content: '是否确定删除？',icon: 'error',ok: function () { art.dialog.open('<?php echo site_url('customer/files_del')?>?id=<?php echo $row['id']?>');return false;},cancel: true }); };</script>
 <script>function File_ContentView<?php echo $row['id']?>() {art.dialog({ title: '详情备注', content: '<?php echo $row['content']?>',drag: false,resize: false}); };</script>
 <script>function View<?php echo $row['id']?>() {art.dialog({ title: '查看图片', content: '<img src="" />',lock: true}); };</script>
 <?php }?>  
 </table>
 </td> 
 </tr>
...
<script>function Add() {$.dialog.open('<?php echo site_url('customer/files_add')?>?id=<?php echo $id?>', {title: '新窗口', width: 500, height: 210,fixed: true}); };</script>
