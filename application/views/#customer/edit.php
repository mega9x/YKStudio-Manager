...
<script language="JavaScript">
    ...
    function CheckInput() {
        ...
        if (<?php echo isset($config['bt_name']) ? 1 : 0?>=="1"
    )
        {
            if ($("#name").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '任务名称不能为空！'});
                $("#name").focus();
                return false;
            }
        }
        ...
        {
            if ($("#type").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '转化情况不能为空！'});
                $("#type").focus();
                return false;
            }
        }

        if (<?php echo isset($config['bt_area']) ? 1 : 0?>=="1"
    )
        {
            if ($("#area1").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '所在地区不能为空！'});
                $("#area1").focus();
                return false;
            }
        }
        ...
        if (<?php echo isset($config['bt_address']) ? 1 : 0?>=="1"
    )
        {
            if ($("#address").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '所在地区不能为空！'});
        ...
        if (<?php echo isset($config['bt_linkman']) ? 1 : 0?>=="1"
    )
        {
            if ($("#linkman").val() == "") {
        ...
        if (<?php echo isset($config['bt_job']) ? 1 : 0?>=="1"
    )
        {
            if ($("#job").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '任务链接不能为空！'});
        ...
        if (<?php echo isset($config['bt_mobile']) ? 1 : 0?>=="1"
    )
        {
            if ($("#mobile").val() == "") {
        ...
        if (<?php echo isset($config['bt_qq']) ? 1 : 0?>=="1"
    )
        {
            if ($("#qq").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '合作站点不能为空！'});
        ...
        if (<?php echo isset($config['bt_trade']) ? 1 : 0?>=="1"
    )
        {
            if ($("#trade").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '任务类型不能为空！'});
                $("#trade").focus();
                return false;
            }
        }
        ...
        {
            if ($("#website").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '公司网址不能为空！'});
                $("#website").focus();
                return false;
            }
        }
        ...
        if (<?php echo isset($config['bt_start']) ? 1 : 0?>=="1"
    )
        {
            if ($("#start").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '流量状况不能为空！'});
                $("#start").focus();
                return false;
            }
        }

        if (<?php echo isset($config['bt_source']) ? 1 : 0?>=="1"
    )
        {
            if ($("#source").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '所属联盟不能为空！'});
                $("#source").focus();
                return false;
            }
        }
        ...
        {
            if ($("#remark").val() == "") {
                art.dialog({title: 'Error', time: 1, icon: 'warning', content: '备注不能为空！'});
                $("#remark").focus();
                return false;
            }
        }
...
<form name="Add" action="<?php echo site_url('customer/edit') ?>?id=<?php echo $id ?>" method="post"
            ...
            <td valign="top" class="td_n pdl10 pdr10 pdt10">
                ...
                ="0" CLASS="table_1">
                    ...
                    <tr>
                        ...
                        <td class="td_r_l" colspan="3"><input type="text" class="int" name="name" id="name" size="50"
                                                              maxlength="50" autocomplete="off"
                                                              value="<?php echo $name ?>"
                                                              onChange="check_customer_name(this.value);"><span
                                    id="customer_name_err"><span class="info_warn help01">客户识别信息</span></span>
                        </td>
                    ...
                    <tr>
                        ...
                        <td class="td_r_l" <?php echo isset($config['hidden_area']) ? 'style="display:none;"' : '' ?>>
                            ...
                            <input name="area1" type="text" class="int" id="area_name1" value="<?php echo $area1 ?>"
                            ...
                            <input name="area2" type="text" class="int" id="area_name2" value="<?php echo $area2 ?>"
                        ...
                        <td class="td_r_l" <?php echo isset($config['hidden_address']) ? 'style="display:none;"' : '' ?>>
                            ...
                            邮编：
                            <input name="Zip" type="text" class="int" id="Zip" size="10" maxlength="6"
                                   onkeyup='this.value=this.value.replace(/\D/gi,"")' value="<?php echo $zip ?>">
                            <span class="info_help help01" onMouseOver="tip.start(this)" title="限：数字">&nbsp;</span>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="td_r_l" <?php echo isset($config['hidden_source']) ? 'style="display:none;"' : '' ?>>
                    ...
                    <tr>
                        ...
                        <td class="td_r_l" <?php echo isset($config['hidden_website']) ? 'style="display:none;"' : '' ?>>
                            <input name="website" type="text" class="int" id="website" size="35"
                                   value="<?php echo $website ?>">
                            <span class="info_help help01" onMouseOver="tip.start(this)"
                                  title="加：http://">&nbsp;</span></td>
                            ...
                            <select name="trade" id="trade">
                                ...
                                <?php foreach ($select_trade as $arr => $row) { ?>
                                    <option value="<?php echo $row['name'] ?>"
                                            id="<?php echo $row['id'] ?>" <?php echo $row['name'] == $trade ? 'selected' : '' ?>><?php echo $row['name'] ?></option>
                                <?php } ?>
                    ...
                    <tr>
                        <td class="td_l_r title">
                            <?php echo !isset($config['bt_linkman']) ? '<font color="#FF0000">*</font>' : '' ?>任务链接
                        </td>
                        <td class="td_r_l"><input name="linkman" type="text" class="int" id="linkman" size="10"
                                                  value="<?php echo $linkman ?>">
                            &nbsp;
                            <?php echo !isset($config['bt_job']) ? '<font color="#FF0000">*</font>' : '' ?>
                            任务链接：
                            <select name="job" id="Select_job">
                                <option value="">请选择</option>
                        ...
                        <td class="td_r_l" colspan="3"><input name="mobile" type="text" class="int" id="mobile"
                                                              size="30" value="<?php echo $mobile ?>"
                    ...
                    <tr>
                        ...
                        <td class="td_r_l"><input name="qq" type="text" class="int" id="qq" size="30"
                                                  onKeyUp="refreshValue(this)" value="<?php echo $qq ?>"></td>
                        <td class="td_l_r title">
                            <?php echo !isset($config['bt_email']) ? '<font color="#FF0000">*</font>' : '' ?>
                            电子邮件
                        </td>
                        <td class="td_r_l"><input name="Email" type="text" class="int" id="Email" size="35"
                    ...
                    <tr <?php echo isset($config['hidden_remark']) ? 'style="display:none;"' : '' ?>>
                        ...
                        <td class="td_r_l" colspan=3 style="padding:5px 10px;">
                            <textarea name="remark" id="remark" class="int"
                                      style="height:50px;width:98%;"><?php echo $remark ?></textarea></td>
                    </tr>
                </table>
            </td>
        </tr>
