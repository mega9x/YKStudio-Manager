...
<form name="Save" id="Level" action="<?php echo site_url('role/edit')?>?id=<?php echo $id?>" method="post" onSubmit="return CheckInput();">
<table width="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <td class="td_l_l"><input name="right" type="text" class="int" id="lId" size="10" maxlength="3" onkeyup='this.value=this.value.replace(/\D/gi,"")' value="<?php echo $right?>" >
 <span class="info_help help02">注：权限值大的可以管理和查看权限值小的帐号任务信息</span></td>
 </tr>
 
 <tr>
 <td colspan="3" class="td_l_r title"><font color="#FF0000">*</font> 角色名称</td>
 <td class="td_l_l"><input name="name" type="text" class="int" id="lName" size="20" maxlength="16" value="<?php echo $name?>" ></td>
 </tr>
 <tr>
 ...
 <input type="button" class="button246" onClick="javascript:selectall('levelA')" <?php echo in_array($row['id'],$lever) ? 'checked' : ''?> value="全选/反选" style="margin-bottom:10px;" />
 ...
 <input type="checkbox" id="levelA" name="lever[]" value="<?php echo $row['id']?>" <?php echo in_array($row['id'],$lever) ? 'checked' : ''?>  class="qxon"/>
 </label>
 </td>
 <?php 
 ...
 <!--<td class="td_l_c fontbold"><?php echo $row['name']?></td>-->
 <td class="td_l_c"><label><?php echo $row['name']?>
 <input type="checkbox" id="levelB" name="lever[]" value="<?php echo $row['id']?>" <?php echo in_array($row['id'],$lever) ? 'checked' : ''?> class="qxon"/>
 </label></td>
 <?php 
  if (isset($row['child']) && count($row['child'])>0) {
  foreach($row['child'] as $arr2=>$row2) {
