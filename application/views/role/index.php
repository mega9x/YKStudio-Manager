...
<div id="setxhing">
    ...
    <ul class="xhing">
                        ...
                        <tr>
                            <td valign="top" class="td_n pdr10 pdt10">
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" CLASS="table_1">
                                    ...
                                    <?php foreach ($list as $arr => $row) { ?>
                                        <tr class="tr">
                                            ...
                                            <td class="td_l_l"><a href='#'
                                                                  onclick='Level_InfoEdit<?php echo $row['id'] ?>()'><?php echo $row['name'] ?></a>
                                            </td>
                                            <td class="td_l_c" nowrap="nowrap">

                                                <input type="button" class="btn1 btnxiug" value="修改" title="修改"
                                                       onclick='Level_InfoEdit<?php echo $row['id'] ?>()'/>
                                                <input class="btn1 btnshanc" type="button"
                                                       onclick="if(confirm(' 你真的要删除吗？')) {location.href='<?php echo site_url('role/del') ?>?id=<?php echo $row['id'] ?>';};"
                                                       title="删除" value="删除">
                                        ...
                                        <script>function Level_InfoEdit<?php echo $row['id']?>() {
                                                $.dialog.open('<?php echo site_url('role/edit')?>?id=<?php echo $row['id']?>', {
                                                    title: '新窗口',
                                                    width: 800,
                                                    height: 480,
                                                    fixed: true
                                    ...
                                    <tr class="tr_t">
                                        <td colspan="4" class="td_l_l"><span class="info_help help02">注：权限值大的可以管理和查看权限值小的帐号客户信息</span>
                                        </td>
                                    </tr>
                                </table>
                            </td>
        ...
        <script>function Level_Add() {
                $.dialog.open('<?php echo site_url('role/add')?>', {
                    title: '新增权限信息',
                    width: 800,
                    height: 600,
                    fixed: true
