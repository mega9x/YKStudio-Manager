...
/*        $split = "-->";
        $preName = array();
        $results = $this->db->query("SELECT * FROM " .$this->db->dbprefix("customer"))->result();
        foreach($results as $row) {
            $name = $row['name'];
            if(strlen($name) <= 0) continue;
            $splitName = explode($split, $name);
            $fullname = trim($splitName[0]);
            if(!array_key_exists($fullname, $preName)) {
                $preName[$fullname] = 0;
            } else {
                $preName[$fullname]++;
            }
            $finalName = $fullname. " " .$split. " " .$preName[$fullname];
            $sql = "UPDATE " .$this->db->dbprefix("customer"). " SET name='" .$finalName. "' WHERE id=" .$row['id'];
            $this->db->query($sql);
        }*/
    }
}