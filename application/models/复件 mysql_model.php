...
class Mysql_model extends CI_Model { 
	...
	public function get_results($table,$where='',$type=2) {
		...
		if (isset($where['order'])) {$this->db->order_by($where['order']);unset($where['order']);}
		if (isset($where['group'])) {$this->db->group_by($where['group']);unset($where['group']);}  
		if (isset($where['limit2'])) {$this->db->limit($where['limit2'], $where['limit1']);unset($where['limit1'],$where['limit2']);} 
		if (isset($where['having'])) {$this->db->having($where['having']); unset($where['having']);} 
