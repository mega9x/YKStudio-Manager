...
class Customer extends CI_Controller
{
    ...
    {
        $this->mysql_model->customerUpdate();
        $this->common_model->checkpurview(19);
        $post = array('mobile', 'type', 'start', 'adduser', 'job', 'linkman', 'state', 'tel','title',
        ...
        $debugStr = $this->mysql_model->setwhere($where);
        $this->console_log($debugStr);
    ...
    public function editAll() {
        ...
        if (!is_array($data) || count($data) <= 0) {
            ...
            $data['select_hetong'] = $this->mysql_model->get_results('set_select', array('isdel' => 0, 'type' => 'hetong'));
            $data['select_source'] = $this->mysql_model->get_results('set_select', array('isdel' => 0, 'type' => 'source'));
            $this->load->view('customer/editall', $data);
            return;
        }
        if(strlen($data['name']) <= 1) {
            str_alert("修改失败: 名字为空 (这会导致删库!)");
            return;
        } else if(strlen($data['name']) <= 5) {
            str_alert("修改失败: 该字段必须有5个字符以上");
            return;
        }
        if(mb_strlen($data['type']) > 0) {
            $edit['type'] = $data['type'];
        }
        if(mb_strlen($data['Start']) > 0) {
            $edit['start'] = $data['Start'];
        }
        if(mb_strlen($data['trade']) > 0) {
            $edit['trade'] = $data['trade'];
        }
        if(mb_strlen($data['qq']) > 0) {
            $edit['qq'] = $data['qq'];
        }
/*        $sql = "UPDATE " .$this->db->dbprefix('customer'). " SET type=".
            $data['type']. "', start='".
            $data['Start']. "', trade='".
            $data['trade']. "', qq='".
            $data['qq']. "' WHERE name like '%". $data['name']. "%'";
        $this->db->query($sql);*/
/*        $this->mysql_model->update('customer', $inset, array('name' => $data['name']));*/
        $where['like|name'] = $data['name'];
        $this->mysql_model->update('customer', $edit, $where);
        $this->load->view('close');
    }
